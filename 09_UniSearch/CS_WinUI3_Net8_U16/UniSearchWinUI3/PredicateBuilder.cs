...
// 2024-05-13   PV      1.9 Julia search for words starting with \
// 2024-09-20   PV      A simple - just search for - characters (has no "not" meaning)
...
internal sealed partial class PredicateBuilder
{
        ...
        else
        {
            ...
            foreach (string oneWord in wordsList)
            {
                ...
                if (isWW)
                {
                    ...
                    word = word.StartsWith("-", StringComparison.OrdinalIgnoreCase)
    ...
    public bool GetBlockNodeFilter(object searched)
    {
        ...
        foreach (string aWord in words)
        {
            ...
            if (aWord.Length > 1 && aWord.StartsWith('-'))
            {
                word = aWord[1..];
                invertFlag = true;
                if (word.Length == 0)
    ...
    private static partial Regex CodepointRangeRegex();
    ...
    public bool GetCharacterRecordFilter(object searched)
    {
        ...
        foreach (string aWord in words)
        {
            ...
            if (UniData.UnicodeLength(word) == 1)
            {
                ...
                {
                    case 0:    // CI AI
                        wordFilter = RemoveDiacritics(cr.Character).Equals(RemoveDiacritics(word), StringComparison.InvariantCultureIgnoreCase);
                        break;
                    case 1:     // CS AI
                        wordFilter = RemoveDiacritics(cr.Character) == RemoveDiacritics(word);
                        break;
                    case 2:     // CI AS
                        wordFilter = cr.Character.Equals(word, StringComparison.InvariantCultureIgnoreCase);
                        break;
                    case 4:     // CS AS
            ...
            {
                wordFilter = cr.CategoryRecord.CategoriesList.Any(s => string.Equals(s, word, StringComparison.OrdinalIgnoreCase));
            }

            // If searched string starts with SC:, it's a script filter
            else if (WordStartsWithPrefix("SC"))
            {
                wordFilter = string.Equals(cr.Script, word, StringComparison.OrdinalIgnoreCase);
            }
            ...
            // If searched word starts with \, it's a Julia filter
            else if (word.StartsWith('\\'))
            {
                wordFilter = cr.Julia.Contains(word[1..], StringComparison.OrdinalIgnoreCase);
            }
            ...
            else
            {
                ...
                bool NameCheck(string name)
                {
                    if (isRE || isWWLocal)
                    {
                        ...
                        {
                            return isAS
                                ? Regex.IsMatch(name, word, isCS ? 0 : RegexOptions.IgnoreCase)
                                : Regex.IsMatch(RemoveDiacritics(name), word, isCS ? 0 : RegexOptions.IgnoreCase);
                        }
                        catch (Exception)
                        {
                            return true;
                    ...
                    {
                        return isAS
                            ? name.Contains(word, isCS ? StringComparison.CurrentCulture : StringComparison.InvariantCultureIgnoreCase)
                            : RemoveDiacritics(name).Contains(word, isCS ? StringComparison.CurrentCulture : StringComparison.InvariantCultureIgnoreCase);
                    }
                }
