...
// 2023-09-21   PV      1.15.1 Unicode 15.1
// 2024-05-13   PV      1.15.2 Added Julia information + code cleanup
// 2024-09-13   PV      Net8 CS12 U16
// 2024-09-20   PV      1.16.1 Added confusables
// 2024-12-08   PV      1.16.2 Net9 C#13
...
public class CharacterRecord
{
    ...
    /// <summary>Julia tab completion string (from julia-tab-completion.txt)</summary>
    public string Julia => _Julia ?? string.Empty;

    internal string? _Julia;
    ...
    /// <summary>Confusables information, a list of sequence of codepoints</summary>
    public List<List<int>>? Confusables { get; internal set; }
    ...
    public string UTF16 => Codepoint is <= 0xD7FF or (>= 0xE000 and <= 0xFFFF) ? Codepoint.ToString("X4") : (0xD800 + ((Codepoint - 0x10000) >> 10)).ToString("X4") + " " + (0xDC00 + (Codepoint & 0x3ff)).ToString("X4");
...
public static partial class UniData
{
    ...
    private static readonly Dictionary<string, CategoryRecord> CatMap = [];
    private static readonly Dictionary<int, BlockRecord> BlockMap = [];
    ...
    public static ReadOnlyDictionary<int, CharacterRecord> CharacterRecords { get; } = new ReadOnlyDictionary<int, CharacterRecord>(CharMap);

    /// <summary>Dictionary of all categories, indexed by category (case sensitive).</summary>
    public static ReadOnlyDictionary<string, CategoryRecord> CategoryRecords { get; } = new ReadOnlyDictionary<string, CategoryRecord>(CatMap);

    /// <summary>Dictionary of all blocks, indexed by 1st Codepoint of the block.</summary>
    public static ReadOnlyDictionary<int, BlockRecord> BlockRecords { get; } = new ReadOnlyDictionary<int, BlockRecord>(BlockMap);
    ...
    private static Stopwatch ReadUnicodeData()
    {
        ...
        using (var sr = new StreamReader(GetResourceStream("UnicodeData.txt")))
            while (!sr.EndOfStream)
            {
                ...
                else if (codepoint is < 32 or (>= 0x7f and < 0xA0))
                ...
                bool isPrintable = codepoint is not (< 32                                // Control characters 0-31
                                    or (>= 0x7f and < 0xA0) or (>= 0xD800 and <= 0xDFFF) or 0x2028                          // U+2028  LINE SEPARATOR
                                    or 0x2029)                          // U+2029  PARAGRAPH SEPARATOR
;
                if (isRange)   // Add all characters within a specified range
                {
                    ...
                    if (codepoint is not 0xF0000 and not 0x100000)
    ...
    private static Stopwatch ReadAges()
    {
        ...
        using (var sr = new StreamReader(GetResourceStream("DerivedAge.txt")))
            while (!sr.EndOfStream)
            {
                ...
                if (p < 0)
                {
                    if (CharMap.TryGetValue(codepoint, out CharacterRecord? value))
                        value.Age = age;
                }
                else
                {
                    ...
                    if (codepoint is not 0xF0000 and not 0x100000)
                        for (int code = codepoint; code <= end_char_code; code++)
                            if (CharMap.TryGetValue(code, out CharacterRecord? value))
                                value.Age = age;
                }
            }
        AgeStopwatch.Stop();
    ...
    private static Stopwatch ReadNamesList()
    {
        ...
        void MergeSubheaders()
        {
            ...
            foreach (string singular in blockSubheaders.Where(s => !s.EndsWith('s')))
                ...
                else if (extraMergeSubheaders.TryGetValue(singular, out string? value))
                {
                    foreach (int cp in blockCodepoints!)
                        if (CharMap[cp].Subheader == singular)
                            CharMap[cp].Subheader = value;
                }
        ...
        using (var sr = new StreamReader(GetResourceStream("NamesList.txt")))
            while (!sr.EndOfStream)
            {
                ...
                else if (line.StartsWith("\t=", StringComparison.Ordinal) || line.StartsWith("\t%", StringComparison.Ordinal))
                {
                    ...
                    if (CharMap[cp16].Synonyms == null)
                        CharMap[cp16].Synonyms = [];
                    CharMap[cp16].Synonyms!.Add(string.Concat(line.Substring(3, 1).ToUpperInvariant(), line.AsSpan(4)));
                }
                else if (line.StartsWith("\tx", StringComparison.Ordinal))
                {
                    ...
                    if (CharMap[cp16].CrossRefs == null)
                        CharMap[cp16].CrossRefs = [];
                    CharMap[cp16].CrossRefs!.Add(string.Concat(crossRef[..1].ToUpperInvariant(), crossRef.AsSpan(1)));
                }
                else if (line.StartsWith("\t*", StringComparison.Ordinal) || line.StartsWith("\t~", StringComparison.Ordinal))
                {
                    // Comments, new in 1.6
                    ...
                    string comment = string.Concat(line.Substring(3, 1).ToUpperInvariant(), line.AsSpan(4));
                    ...
                    if (CharMap[cp16].Comments == null)
                        CharMap[cp16].Comments = [];
    ...
    /// <summary>Read Julia tab completion codes from julia-tab-completion.txt.</summary>
    private static Stopwatch ReadJulia()
    {
        var scriptsStopwatch = Stopwatch.StartNew();
        using (var sr = new StreamReader(GetResourceStream("julia-tab-completion.txt")))
            while (!sr.EndOfStream)
            {
                string line = sr.ReadLine() ?? string.Empty;
                if (line.Length == 0 || line[0] == '#')
                ...
                // Ignore composed Julia names such as U+0224F + U+00338	≏̸	\nbumpeq
                if (fields[0].Length == 7 && fields[0].StartsWith("U+"))
                {
                    int codepoint = int.Parse(fields[0][2..], NumberStyles.HexNumber);
                    string julia = fields[2];
                    if (CharMap.TryGetValue(codepoint, out CharacterRecord? cr))
                        cr._Julia = julia;
                }
            }
    ...
    private static Stopwatch ReadConfusables()
    {
        var scriptsStopwatch = Stopwatch.StartNew();

        var lll = new List<List<List<int>>>();
        var lastField1 = "";

        var ll = new List<List<int>>();
        using (var sr = new StreamReader(GetResourceStream("confusables.txt")))
            while (!sr.EndOfStream)
            {
                string line = sr.ReadLine() ?? string.Empty;
                if (line.Length == 0 || line[0] == '#')
                ...
                if (field1 != lastField1)
                {
                    ll = [];
                    var l = new List<int>();
                    string[] fields1 = field1.Split(" ", StringSplitOptions.RemoveEmptyEntries);
                    foreach (var item in fields1)
                    {
                        int target = int.Parse(item.Trim(), NumberStyles.HexNumber);
                        l.Add(target);
                    }
                    ll.Add(l);
                    lll.Add(ll);
                ...
                int source = int.Parse(fields[0].Trim(), NumberStyles.HexNumber);
                var ls = new List<int>() { source };
                ll.Add(ls);
            }

        // Add confusables groups to characters map
        foreach (var cll in lll)
            foreach (var cl in cll)
                if (cl.Count == 1)
                    CharMap[cl[0]].Confusables = cll;
    ...
    public static string AsString(int cp) => cp is <= 0xD7FF or (>= 0xE000 and <= 0xFFFF) ? new string((char)cp, 1) : new string((char)(0xD800 + (cp - 0x10000 >> 10)), 1) + new string((char)(0xDC00 + (cp & 0x3ff)), 1);
    ...
    public static int UnicodeLength(string str)
    {
        ...
        foreach (char c in str)
        {
            ...
            {
                l++;
                isSurrogate = c is >= (char)0xD800 and <= (char)0xDBFF;
            }
        }
        return l;
    }
