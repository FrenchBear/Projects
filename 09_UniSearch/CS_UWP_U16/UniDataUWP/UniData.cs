...
namespace UniDataNS
{
    ...
    public static class UniData
    {
        ...
        private static readonly Dictionary<int, CharacterRecord> CharMap = new Dictionary<int, CharacterRecord>(65536);
        private static readonly Dictionary<string, CategoryRecord> CatMap = new Dictionary<string, CategoryRecord>();
        private static readonly Dictionary<int, BlockRecord> BlockMap = new Dictionary<int, BlockRecord>();
            ...
            using (var sr = new StreamReader(GetResourceStream("UnicodeData.txt")))
                while (!sr.EndOfStream)
                {
                    ...
                    if (codepoint == 28)
                        ...
                        charName = "CONTROL - " + (fields[10].Length > 0 ? fields[10] : fields[0].Substring(2));
            ...
            foreach (var br in BlockMap.Values)
                for (int ch = br.Begin; ch <= br.End; ch++)
                    if (CharMap.ContainsKey(ch))
                        CharMap[ch].BlockBegin = br.Begin;
        }
            ...
            using (var sr = new StreamReader(GetResourceStream("NamesList.txt")))
                while (!sr.EndOfStream)
                {
                    ...
                    else if (line.StartsWith("\t=", StringComparison.Ordinal) || line.StartsWith("\t%", StringComparison.Ordinal))
                    {
                        ...
                        CharMap[cp16].Synonyms!.Add(line.Substring(3, 1).ToUpperInvariant() + line.Substring(4));
                    }
                    else if (line.StartsWith("\tx", StringComparison.Ordinal))
                    {
                        ...
                        CharMap[cp16].CrossRefs!.Add(crossRef.Substring(0, 1).ToUpperInvariant() + crossRef.Substring(1));
                    }
                    else if (line.StartsWith("\t*", StringComparison.Ordinal) || line.StartsWith("\t~", StringComparison.Ordinal))
                    {
                        // Comments, new in 1.6
                    ...
                    else
                    {
                        ...
                        if (CharMap.ContainsKey(cp16))
                        {
                            ...
                            CharMap[cp16].Subheader = subheader;
                        }
                    }
                }
            if (blockCodepoints != null)
        ...
        private static Stopwatch ReadScripts()
        {
            ...
            using (var sr = new StreamReader(GetResourceStream("Scripts.txt")))
                while (!sr.EndOfStream)
                {
                    ...
                    {
                        if (CharMap.ContainsKey(codepoint))
                            CharMap[codepoint]._Script = script;
                    }
                    else
                    {
                        int end_char_code = int.Parse(fields[0].Substring(p + 2), NumberStyles.HexNumber);
        ...
        private static Stopwatch ReadJulia()
        {
            ...
            using (var sr = new StreamReader(GetResourceStream("julia-tab-completion.txt")))
                while (!sr.EndOfStream)
                {
                    ...
                    {
                        int codepoint = int.Parse(fields[0].Substring(2), NumberStyles.HexNumber);
        ...
        private static Stream GetResourceStream(string name)
        {
            ...
            return qualifiedName == null
                ...
                : assembly.GetManifestResourceStream(qualifiedName);
        }
