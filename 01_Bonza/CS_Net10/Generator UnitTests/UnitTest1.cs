...
public class UnitTest1
{
    ...
    {
        Assert.AreEqual(PlaceWordStatus.Valid, g.Layout.AddWordPosition(new WordPosition("PIERRE", "Pierre", new PositionOrientation(0, 0, false))));
        Assert.AreEqual(PlaceWordStatus.Valid, g.Layout.AddWordPosition(new WordPosition("VIOLENT", "Violent", new PositionOrientation(-1, 1, true))));
        Assert.AreEqual(PlaceWordStatus.TooClose, g.Layout.AddWordPosition(new WordPosition("BOB", "Bob", new PositionOrientation(1, 0, false))));
        Assert.AreEqual(PlaceWordStatus.Invalid, g.Layout.AddWordPosition(new WordPosition("JOE", "Joe", new PositionOrientation(3, 0, false))));
    }
    ...
    public void TestAddWordPositionAndSquares2()
    {
        ...
        Assert.AreEqual(PlaceWordStatus.Valid, g.Layout.AddWordPosition(wp));
        Assert.AreEqual(PlaceWordStatus.Valid, g.Layout.AddWordPosition(new WordPosition("VIOLENT", "Violent", new PositionOrientation(-1, 1, true))));
        // WordPosition already placed, must raise ArgumentException
        Assert.Throws<ArgumentException>(() => g.Layout.AddWordPosition(wp));
    }
