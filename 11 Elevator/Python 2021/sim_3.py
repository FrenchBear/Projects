# Simulator 2021
# v3, with regular time slices and storyboards
...
class Storyboard:
    def __init__(self, a:Actor, state: str, next_state: str) -> None:
        self.start_transition_clock = a.act_clock
        a.state = state
        self.state = state
        self.next_state = next_state

    def set_clock(self, a:Actor, dt: datetime) -> bool:
...
class Wait(Storyboard):
    ...
    def set_clock(self, a:Actor, dt: datetime) -> bool:
        super().set_clock(a, dt)
        if self.t<self.tmax:
            return False
        a.state = self.next_state
        return True
...
class Car(Actor):
    ...
    def set_clock(self, dt: datetime):
        ...
        if self.storyboard:
            res = self.storyboard.set_clock(self, dt)
            if res:
