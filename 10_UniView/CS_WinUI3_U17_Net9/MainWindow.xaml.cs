...
//
// 2023-08-25   PV
// 2023-09-19   PV      Unicode 15.1
// 2023-11-20   PV      Net8 C#12 (was previously using Net6)
// 2024-09-15   PV      Unicode 16; Code cleanup
// 2024-09-27   PV      Use AppWindow.SetIcon to set main window icon; removed NativeMethods
// 2024-11-29   PV      1.3.0 Net9 C#13 (to get rid of warning CsWinRT1029 Class implements WinRT interfaces generated using an older version of CsWinRT)
// 2024-12-03   PV      1.3.1 Sort by CodepointIndex when copying list data to clipboard
// 2024-12-21   PV      1.3.2 Show UTF-8 bytes count
// 2025-10-07   PV      1.17.0 Unicode 17
...
using UD = UniView_WinUI3.UniData.UniData;
...
public sealed partial class MainWindow: Window
{
    ...
    public MainWindow()
    {
        ...
        appWindow.Resize(new Windows.Graphics.SizeInt32 { Width = 1200, Height = 1024 });
        appWindow.Title = "UniView WinUI3";

        AppWindow.SetIcon(@"Assets\Cat2.ico");
    ...
    private async Task DoAbout()
    {
        ...
        s += "\n\nUnicode Data: " + UD.GetUnicodeVersion();
    ...
    private void Transform()
    {
        ...
        static string ReplaceRange(Match ma)
        {
            if (int.TryParse(ma.Groups[1].ToString(), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out int cpFrom))
                ...
                {
                    if (cpFrom > UD.MaxCodepoint || cpTo > UD.MaxCodepoint)
                    ...
                    for (int cp = cpFrom; cp <= cpTo; cp++)
                        if (!UD.IsSurrogate(cp))   // For ranges, we silently ignore surrogates to enable large ranges including U+D800..U+DFFF
                        {
                            sb.Append(UD.AsString(cp));
        ...
        static string ReplaceCodepoint(Match ma)
        {
            ...
            {
                return cp > UD.MaxCodepoint
                    ? "[Codepoint limit is 10FFFF]"
                    : UD.IsSurrogate(cp) ? "[Invalid codepoint in surrogates range D800..DFFFF]" : UD.AsString(cp);
            }
        ...
        s = reName.Replace(s, ma =>
        {
            ...
            int cp = UD.GetCpFromName(name);
            if (cp >= 0)
                return UD.AsString(cp);
        ...
        // Update counts
        VM.BytesCount = Encoding.UTF8.GetByteCount(Transformed);
    ...
    private static List<CodepointDetail> GetCodepointsDetails(ref string s, bool expandEmoji)
    {
        ...
        for (int i = 0; i < s.Length; i++)
        {
            ...
            if (expandEmoji && UD.CharacterRecords.TryGetValue(cp, out var cr) && cr.IsEmojiXXX)
            {
                string n = "{" + UD.GetName(cp) + "}";
                foreach (char c in n)
    ...
    private async void SearchButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            XamlRoot = Content.XamlRoot     // Actually, this.Content.XamlRoot
        };
