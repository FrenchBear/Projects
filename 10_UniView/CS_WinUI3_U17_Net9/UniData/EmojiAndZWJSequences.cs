...
public static class EmojiAndZWJSequences
{
    ...
    private static readonly List<UnicodeSequence> EmojiSequenceList = [];
    ...
    private static Stopwatch ReadEmojiSequences()
    {
        ...
        using (var sr = new StreamReader(UniData.GetResourceStream("emoji-sequences.txt"), Encoding.UTF8))
            while (!sr.EndOfStream)
            {
                ...
                int[] sequence = [.. fields[0].Split(' ', StringSplitOptions.RemoveEmptyEntries).Select(x => int.Parse(x, NumberStyles.HexNumber))];
    ...
    internal static UnicodeSequence? GetSequenceFromName(string name)
    {
        ...
        return EmojiSequenceList.FirstOrDefault(s => string.Equals(name, s.CanonizedName, StringComparison.CurrentCultureIgnoreCase));
    }
}