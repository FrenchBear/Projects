...
internal sealed partial class SearchViewModel: INotifyPropertyChanged
{
    ...
    public static bool WindowLoaded;
    ...
    public void ApplyFilter()
    {
        ...
        foreach (UnicodeSequence item in _Matches.Where(fpb.GetFilter))
    ...
    public ObservableCollection<UnicodeSequence> MatchesFilteredList { get; set; } = [];
    ...
    internal void SetOutputProperty(ref bool? field, bool? newValue, [CallerMemberName] string propertyName = "?")
    {
        if (!WindowLoaded)
            return;
        if (field != newValue)
