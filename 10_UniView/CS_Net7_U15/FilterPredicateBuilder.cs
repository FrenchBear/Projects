    ...
    public bool GetFilter(object searchedObject)
    {
        ...
        switch (searched.SequenceType)
        {
            ...
            case SequenceType.EmojiSequence:
                if (EmojiSequenceFilter is {Flag:false, Keycap:false, Modifier:false}) return false;
                if (searched.SequenceSubtype == SequenceSubtype.SequenceFlag && !EmojiSequenceFilter.Flag) return false;
                if (searched.SequenceSubtype == SequenceSubtype.SequenceKeycap && !EmojiSequenceFilter.Keycap) return false;
                if (searched.SequenceSubtype == SequenceSubtype.SequenceModifier && !EmojiSequenceFilter.Modifier) return false;
                break;

            case SequenceType.ZWJSequence:
                if (ZWJSequenceFilter is {Family:false, Roles:false, Gendered:false, Hair:false, Other:false }) return false;
                if (searched.SequenceSubtype==SequenceSubtype.ZWJSequenceFamily && !ZWJSequenceFilter.Family) return false;
                if (searched.SequenceSubtype == SequenceSubtype.ZWJSequenceRoles && !ZWJSequenceFilter.Roles) return false;
                if (searched.SequenceSubtype == SequenceSubtype.ZWJSequenceGendered && !ZWJSequenceFilter.Gendered) return false;
                if (searched.SequenceSubtype == SequenceSubtype.ZWJSequenceHair && !ZWJSequenceFilter.Hair) return false;
                if (searched.SequenceSubtype == SequenceSubtype.ZWJSequenceOther && !ZWJSequenceFilter.Other) return false;
                break;
        }
