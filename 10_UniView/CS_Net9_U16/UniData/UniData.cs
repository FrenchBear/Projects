...
namespace UniView_CS_Net8.UniData;
...
public static class UniData
{
    ...
    public static string GetName(int cp) => CharMap.TryGetValue(cp, out CharacterRecord? value) ? value.Name : $"Unassigned codepoint - {cp:X4}";

    public static string GetCategory(int cp) => CharMap.TryGetValue(cp, out CharacterRecord? value) ? value.Category : "??";

    public static string GetScript(int cp) => CharMap.TryGetValue(cp, out CharacterRecord? value) ? value.Script : "Unknown";
        ...
        using (var sr = new StreamReader(GetResourceStream("UnicodeData.txt"), Encoding.UTF8))
            while (!sr.EndOfStream)
            {
                ...
                bool isPrintable = codepoint is not (< 32           // Control characters 0-31
                                    or (>= 0x7f and < 0xA0)         // Control characters 127-160
                                    or (>= 0xD800 and <= 0xDFFF)    // Surrogates
                                    or 0x2028                       // U+2028  LINE SEPARATOR
...
;
    ...
    private static void ReadUnicodeVersion()
    {
        ...
        while (!sr.EndOfStream)
        {
            if (!string.IsNullOrEmpty(_UnicodeVersion))
                _UnicodeVersion += "\r\n";
            _UnicodeVersion += sr.ReadLine() ?? "";
        }
    }
