...
using static UniView_CS_Net10.CurrentSearchOptions;
...
internal sealed class SearchViewModel: INotifyPropertyChanged
{
    ...
    {
        // Add codepoints
        foreach (var cr in UniData.UniData.CharacterRecords.Values)
        {
            ...
            Matches.Add(new UnicodeSequence(cr.Name, [cr.Codepoint], sequenceType, sequenceSubtype));
        }

        // Add Emoji and ZWJ sequences
        foreach (var es in UniData.EmojiAndZWJSequences.EmojiSequenceRecords)
            Matches.Add(es);
    ...
    {
        if (CollectionViewSource.GetDefaultView(Matches) is ListCollectionView view)
        {
            var fpb = new FilterPredicateBuilder(Filter, false, false, SymbolFilter, ScriptFilter, SequenceFilter, ZWJFilter);
    ...
    public ObservableCollection<UnicodeSequence> Matches { get; } = [];
    ...
    private void UpdateEmojiSequenceCheckBox(string index, bool value)
    {
        ...
        emojiSequenceAll = EmojiSequenceDictionary.Values.All(b => b) ? true : EmojiSequenceDictionary.Values.All(b => !b) ? false : null;
        NotifyPropertyChanged(nameof(EmojiSequenceAll));
        IgnoreClick = false;
        ApplyFilter();
    }
    ...
    private void UpdateZWJSequenceCheckBox(string index, bool value)
    {
        ...
        zwjSequenceAll = ZWJSequenceDictionary.Values.All(b => b) ? true : ZWJSequenceDictionary.Values.All(b => !b) ? false : null;
        NotifyPropertyChanged(nameof(ZWJSequenceAll));
        IgnoreClick = false;
        ApplyFilter();
    }
    ...
    internal void SetOptionProperty(ref bool? field, bool? newValue, [CallerMemberName] string propertyName = "?")
    {
        if (field != newValue)
        {
            field = newValue;
