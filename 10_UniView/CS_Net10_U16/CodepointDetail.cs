...
// 2024-09-15   PV      2.3.0   Unicode 16; Code cleanup
// 2026-01-20	PV		Net10 C#14

using UD = UniView_CS_Net10.UniData.UniData;
...
/// </summary>
internal sealed class CodepointDetail
{
    ...
    public string Name => UD.GetName(Codepoint);

    public string Category => UD.GetCategory(Codepoint);

    public string Script => UD.GetScript(Codepoint);

    // Normal string representation of the Codepoint, using 2 surrogates for chars out of BMP
    public override string ToString() => UD.AsString(Codepoint);

    // Same as ToString, with the exception of most control characters 0..31 and 127 replaced by a visual representation ␀..␟ and ␡
    public string ToDisplayString()
    {
        if (Codepoint is < 32 and not 9 and not 10 and not 13)   // TAB, CR and LF keep their default representation
            return UD.AsString(Codepoint + 0x2400);
        else if (Codepoint == 127)
            return UD.AsString(0x2421);
        else if (Codepoint == 0x85)     // NEXT LINE character has no visual glyph, replace it by LineFeed
            return UD.AsString(10);
            ...
            return UD.AsString(10);
        else if (Codepoint is >= 0xFFF9 and <= 0xFFFB || UD.IsNonCharacter(Codepoint))
    ...
    public string ToDisplayStringFull()
    {
        if (Codepoint < 32)   // All control characters, including TAB, CR and LF are replaced by a visual equivalent
            ...
            return UD.AsString(0x2424);
        else if (Codepoint == 0xAD)     // SOFT HYPHEN character has no visual glyph, replace it by -
            return UD.AsString('-');
        else if (Codepoint is >= 0x2028 and <= 0x2029)
            // 2028  LINE SEPARATOR
            // 2029  PARAGRAPH SEPARATOR
            return UD.AsString(0xFFFD);    // � Replacement character
        else if (Codepoint is >= 0xFFF9 and <= 0xFFFB || UD.IsNonCharacter(Codepoint))
            // FFF9  INTERLINEAR ANNOTATION ANCHOR
            // FFFA  INTERLINEAR ANNOTATION SEPARATOR
            // FFFB  INTERLINEAR ANNOTATION TERMINATOR
    ...
    public bool IsBMP => Codepoint is <= 0xD7FF or (>= 0xE000 and <= 0xFFFF);
