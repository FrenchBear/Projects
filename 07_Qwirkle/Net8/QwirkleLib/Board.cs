...
public partial class Board
{
    ...
    {
        if (Base.dict.TryGetValue(coord, out var s))
            Debug.Assert(s.State == SquareState.Playable || s.State == SquareState.Unknown || s.State == SquareState.Empty);

        Base.dict[coord] = value;
        if (coord.row < Base.RowMin)
            Base.RowMin = coord.row;
        if (coord.row > Base.RowMax)
            Base.RowMax = coord.row;
        if (coord.col < Base.ColMin)
            Base.ColMin = coord.col;
        if (coord.col > Base.ColMax)
            Base.ColMax = coord.col;
    }

    public void SetPlayedSquare((int row, int col) coord, Square value)
    {
        // Can't play over a tiled board square
        if (Base.dict.TryGetValue(coord, out Square? s1))
            Debug.Assert(s1.Tile == null);
        // Can't play over a tiled or blocked played square
        if (Played.dict.TryGetValue(coord, out Square? s2))
            Debug.Assert(s2.State == SquareState.Playable || s2.State == SquareState.Unknown || s2.State == SquareState.Empty);
        ...
        if (coord.row < Played.RowMin)
            Played.RowMin = coord.row;
        if (coord.row > Played.RowMax)
            Played.RowMax = coord.row;
        if (coord.col < Played.ColMin)
            Played.ColMin = coord.col;
        if (coord.col > Played.ColMax)
            Played.ColMax = coord.col;
    }
    ...
    public void RollbackPlay() => ClearPlayedDict();
    ...
    private void FindAndResetSquarePlayability((int row, int col) coord, int deltaRow, int deltaCol, bool isPlayed)
    {
        ...
        if (s.State == SquareState.Blocked)
            return;     // Blocked state is final
        if (s.State == SquareState.Unknown)
            return;     // Already unknown, won't change
    ...
    private (ShapeConstraint, ColorConstraint) GetConstraintsFromDirection((int row, int col) coord, int deltaRow, int deltaCol, bool isPlayed)
    {
        ...
        for (; ; )
        {
            ...
            if (s.Tile == null)
                return (shapeConstraint, colorConstraint);
    ...
    private int GetTilesCount((int row, int col) coord, int deltaRow, int deltaCol, bool isInRow)
    {
        ...
        for (; ; )
        {
            ...
            if (!IsTiled(coord))
                return n;
            n++;
            if (Played.dict.TryGetValue(coord, out Square? value))
            {
                ...
                Debug.Assert(value.Tile != null);

                if (isInRow)
                {
                    Debug.Assert(!value.pointsInRow);
                    value.pointsInRow = true;
                }
                else
                {
                    Debug.Assert(!value.pointsInCol);
                    value.pointsInCol = true;
                }
            }
        }
    }
